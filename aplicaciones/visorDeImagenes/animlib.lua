...
function anim.createloadtable(rutaprefijo, inicio, fin, extension, cantcf)
	...
	else cantcf = "%d" end -- sino, el número tal cual (Ej: 1 2 3 4 5 6...)
...
function anim.create(imgs, w, h, tiempo)
	...
	elseif args == 2 then -- anim.create(imgs,tiempo) -- tabla con imagenes o rutas o variadas, y tiempo entre cada cuadro
		for i=1,#imgs do
			...
			else -- supongo que es un número
				animobj.tim[i] = tiempo -- todos los frames la misma duración
			end
		end
		...
		for j=1,ch do
			for i = 1,cw do
				...
				if tbltim then -- si los tiempos son una tabla
					...
					animobj.tim[indice] = tiempo -- todos los frames la misma duración
				end
				indice = indice + 1
			end
		end
...
function anim.loadgif(ruta)  -- carga un gif animado y retorna una tabla con los frames y otra con la duración de cada frame en milisegundos (ahora integrada a animlib)
